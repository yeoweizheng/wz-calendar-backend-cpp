...
vector<ScheduleItem> getScheduleItems(int userId, string searchStr) {
    ...
    vector<ScheduleItem> scheduleItems;
    searchStr = "%" + searchStr + "%";
    db << "SELECT id, name, date, done, tag_id FROM schedule_scheduleitem WHERE user_id = ? AND done = false AND name LIKE ? ORDER BY date ASC;"
        << userId << searchStr
...
void checkScheduleItemExists(database db, int userId, int scheduleItemId) {
    db << "SELECT COUNT(id) FROM schedule_scheduleitem WHERE id = ? AND user_id = ?;"
...
ScheduleItem getScheduleItem(int userId, int scheduleItemId) {
    ...
    db << "SELECT id, name, date, done, tag_id FROM schedule_scheduleitem WHERE user_id = ? AND id = ?;"
...
vector<Tag> getTags(int userId) {
    database db(DB_FILENAME);
    vector<Tag> tags;
    db << "SELECT id, name FROM schedule_tag WHERE user_id = ?;"
        << userId
        >> [&](int id, string name) {
            Tag tag = { id, name };
            tags.push_back(tag);
...
int insertTag(int userId, string name) {
    database db(DB_FILENAME);
    db << "INSERT INTO schedule_tag (user_id, name) VALUES (?, ?);"
        << userId << name;
    return db.last_insert_rowid();
}

void checkTagExists(database db, int userId, int tagId) {
    db << "SELECT COUNT(id) FROM schedule_tag WHERE id = ? AND user_id = ?;"
        << tagId << userId
        >> [](int count) {
            if (count != 1) { throw runtime_error("Not found"); }
        };
...
Tag getTag(int userId, int tagId) {
    ...
    Tag tag;
    db << "SELECT id, name FROM schedule_tag WHERE user_id = ? AND id = ?;"
        << userId << tagId
        >> [&](int id, string name) {
            tag = { id, name };
        };
...
void updateTag(int userId, int tagId, string name) {
    database db(DB_FILENAME);
    checkTagExists(db, userId, tagId);
    db << "UPDATE schedule_tag SET name = ? WHERE id = ? AND user_id = ?;"
        << name << tagId << userId;
}
...
void deleteTag(int userId, int tagId) {
    database db(DB_FILENAME);
    checkTagExists(db, userId, tagId);
    db << "DELETE FROM schedule_tag WHERE id = ? AND user_id = ?;"
        << tagId << userId;
}